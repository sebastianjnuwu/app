...
export async function LEAVE_ROOM(
    ...
    if (!ROOM || !PLAYER || Object.keys(ROOM).length === 0 || Object.keys(PLAYER).length === 0) return;
    ...
    if (ROOM.STATE !== "WAITING") {
        ...
        logger.info(`🚪 Player ${chalk.green(`"${USER.NAME}"`)} left room ${chalk.cyanBright(`"${ROOM_CODE}"`)} (state: ${chalk.yellow(ROOM.STATE)})`);
        return;
    }
    ...
    if (remainingPlayers.length === 0) {
        ...
        socket.leave(ROOM_CODE);
        logger.info(`🗑️  Room ${chalk.cyanBright(`"${ROOM_CODE}"`)} deleted. Last owner: ${chalk.green(`"${USER.NAME}"`)}`);
        return;
    }
