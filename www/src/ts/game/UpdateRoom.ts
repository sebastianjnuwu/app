...
socket.on("UPDATE_ROOM", ({ TYPE, PLAYER, ROOM }: { TYPE: string; PLAYER: { NAME: string }; ROOM: any }) => {
  if (!ROOM) return;
  ...
  const playerStillInRoom = ROOM.PLAYERS?.some((p: any) => p.UID === localPlayer?.UID);

  if (!playerStillInRoom) {
    $("#GAMEPLAY").attr("hidden", "true");
    $("#home_menu").removeAttr("hidden");
    showMessage(`<i class="fas fa-sign-out-alt"></i> ${lang("app.room.message.leave", { room_player: localPlayer?.NAME })}`);
  ...
  switch (TYPE) {
    ...
    case "OWNER_CHANGED":
      ...
      showMessage(`<i class="fas fa-crown"></i> ${lang("app.room.message.owner_changed")}`);
      break;
  }
});
